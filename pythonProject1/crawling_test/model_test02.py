...
def sentiment_predict(new_sentence):
    ...
    print(p_score)
    if score > 0.97:
        print("{:.2f}/100점으로 맛집입니다.".format((score * 10-p_score)*100))
    elif score > 0.95:
        print("{:.2f}/100점으로 평범합니다.".format((score * 10-p_score)*100))
    else:
        print("{:.2f}/100점으로 조심하세요.".format((score * 10-p_score)*100))
...
while True:
    keyword = input('식당을 입력 해주세요 종료 Q >> ')
    if keyword == 'Q' or keyword == 'q' or keyword == 'ㅂ':
        print('프로그램 종료')
        break
    url = geturl(keyword)
    if url == 'fail':
        print('존재하지 않는 식당입니다. 다시 입력 해주세요')
    else:
        cur.execute(query, [url])
        rows = cur.fetchall()
        print(len(rows))
        if len(rows) > 0:
            sentiment_predict(str(rows))
        else:
            ...
            print(search_data)
            sentiment_predict(str(search_data))
conn.close()
